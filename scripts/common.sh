...
# The name of the stack to manage.
pulumi_stack_name() {
...
# run_pulumi_stack_select selects the appropriate Pulumi stack.
...
# run_pulumi determines whether
run_pulumi() {
    if [ "${pulumi_stack_name}" == "production" ]; then
        assume-role $(run_pulumi_config get roleArn) pulumi -C infrastructure ${1}
    else
        pulumi -C infrastructure ${1}
    fi
}
